...
func (r *ToolsRegistry) RegisterToolProvider(toolProvider types.ToolProvider) error {
...
func (r *ToolsRegistry) Prepare(ctx context.Context, toolConfigs map[string]config.ToolConfig) error {
	...
	for _, toolProvider := range r.ToolProviders {
		...
		for _, tool := range tools {
			...
			toolPath := fmt.Sprintf("%s.%s", toolProvider.Name(), tool.Name())
			r.Tools[toolPath] = tool
		}
	}
